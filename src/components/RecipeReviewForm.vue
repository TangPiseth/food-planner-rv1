<template>
    <div class="review-form-compact" data-aos="fade-up">
      ...
      <div v-else>
        <!-- Alert messages -->
        <div v-if="showAlert" :class="['alert', alertType === 'success' ? 'alert-success' : 'alert-danger', 'mb-3']" role="alert">
          <i :class="['fa-solid', alertType === 'success' ? 'fa-check-circle' : 'fa-exclamation-circle', 'me-2']"></i>{{ alertMessage }}
        </div>
        ...
        <form @submit.prevent="handleSubmit">
          ...
          <div class="mb-3">
            ...
            <div class="text-danger small mt-1" v-if="errors.rating">
              {{ errors.rating }}
            </div>
          </div>
    ...
    methods: {
      async checkExistingReview() {
        ...
        if (result.success && result.hasReviewed && result.review) {
          ...
          this.existingReviewId = result.review.id;
      ...
      async handleSubmit() {
        ...
        if (result.success) {
          ...
          // If it was a new review, switch to edit mode
          if (!this.isEditMode) {
            this.isEditMode = true;
            this.existingReviewId = result.review.id;
          }
